...
export class Webview {
	...
	#callbacks: Map<string, Deno.UnsafeCallback<{ parameters: 'pointer'[]; result: 'void' }>> =
		new Map();
	 ...
	 * here.
	 */
	constructor(debug?: boolean, size?: Size, window?: Deno.PointerValue | null);
	constructor(
	...
	/**
	 * Terminate the main event loop ("main_loop"), with subsequent automatic destruction of the webview.
	 */
	terminate(): void {
	...
	bindRaw(
		name: string,
		callback: (seq: string, req: string, arg: Deno.PointerValue | null) => void,
		arg: Deno.PointerValue | null = null,
	) {
		const callbackResource = new Deno.UnsafeCallback({
			parameters: ['pointer', 'pointer', 'pointer'],
			result: 'void',
		}, (seqPtr: Deno.PointerValue, reqPtr: Deno.PointerValue, arg: Deno.PointerValue | null) => {
